...
class UpdateEnterpriseRequest extends FormRequest
{
    ...
    {
        $enterpriseId = $this->route('record'); // Para Filament, ajustar según necesidad

        return [
            'name' => 'required|string|max:255',
            'tax_id' => 'required|integer|digits:' . config('config-app.enterprise_id_digits') . '|unique:enterprises,tax_id,' . $enterpriseId,
        ];
    }
    ...
    public function messages(): array
    {
        return [
            ...
            'tax_id.unique' => 'El ID fiscal ya existe.',
        ];
    }
}
